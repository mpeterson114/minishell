...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/12 17:00:08 by ilzhabur          #+#    #+#             */
/*   Updated: 2023/11/14 11:54:07 by mpeterso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int len, char **new_tab, t_command *last_cmd, t_token **tk_node)
{
	...
	while (temp->type == WORD || temp->type == VAR)
	{
		new_tab[i] = ft_strdup(temp->content);
		i++;
		temp = temp->next;
	}
	new_tab[i] = NULL;
...
int	fill_command_args(t_token **token_node, t_command *last_cmd)
{
	...
	else
	{
		if (last_cmd && !(last_cmd->args))
			return (create_cmdargs_standard(token_node, last_cmd));
		else
